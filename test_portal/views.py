...
def login(request):
    # handles logic to login students
...
def ques_detail_mcq(request, ques_no=1, id="a"):
    ...
    # Remember to hardcode this value.
    ...
    try:
        ...
        form = GetResponseMCQ(
            initial={
                ...
                "response3": new.response3,
                "response4": new.response4,
            }
        )
        res1 = new.response1
        res2 = new.response2
    ...
    return render(
        request,
        "test_portal/round1.html",
        {
            "question": question,
            "form": form,
            "pksent": ques_no,
            "id": id,
            "n": range(1, ques_count + 1),
            "time": time,
            "curr_time": curr_time,
            "response1": res1,
            "response2": res2,
...
def response_savem(request, pk, next, id="a"):
    if request.method == "POST":
        ...
        if response_rec.is_valid():
            ...
            response.question = QuestionMCQ.objects.get(ques_no=pk)

            if (
                (response.response1 == response.question.ans1)
                and (response.response2 == response.question.ans2)
                and (response.response3 == response.question.ans3)
                and (response.response4 == response.question.ans4)
            ):
